...
LR (学習率) の分析結果  
最大 LR: 7.5309 × 1e-4 (基準の 1e-4 と比較して、約 7.5 倍もの高出力を瞬間的に出している)  
平均 LR: 2.0752 × 1e-4 (全期間を通しても、基準の約 2 倍の強さで学習し続けている)  
最低 LR: 6.2770 × 1e-6 (全期間で 1e-6 に達した回数は数回だけで持続もしない(前半に集中))  

面積 (学習総量) の比較基準を｢1e-4 の Cosine｣とした場合、結果は以下の通り  
emoPulse の総面積: 0.0830083  
Cosine (1e-4) の総面積: 0.0254647  
比較比率: 325.97%  

後半に｢2.92e-4｣あたりで停滞 (READY TO STOP 以降) していた時間は、基準の 3 倍近い出力  
400 ステップという短期間で、通常の 1200〜1300 ステップ分くらいの｢学習｣を進行したことになる  
